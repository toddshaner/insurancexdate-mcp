...
| `company_details` | $0.25 (gated) | Full account detail by UID: `summary`, `user_status`, `details` (field count varies by record: ~77 observed 2026-06-12, 46 on records probed 2026-07-03), `carrier_history[]` (per-policy-term rows, multi-year multi-state), `_meta` field docs. Optional `scope` blocks VERIFIED in paid responses 2026-07-03: `contacts` (names/emails/phones/LinkedIn), `altloc` → `other_locations`, `tabs` → osha + benefits blocks + a `dot[]` block on DOT-flagged companies (verified on a live trucking risk: 35 MCS-150 fields incl. safety rating, power units, drivers, coverage, policy number), `comments`. 90-day dedupe vendor-documented in 2 written sources (unreceipted) |
| `talkpoints` | $0.10 (gated) | Prospecting talking points + percentile flags by UID. 90-day dedupe vendor-documented in 2 written sources (unreceipted) |
| `serff_search` | Conflicting vendor pricing 2026-07-03 ($0.05 per official pricing page vs free per tools/list metadata) — treat as $0.05, gated | SERFF rate-filing search. `carrier_naic` now optional (statewide all-carrier queries verified); severity comma-lists, `policyholders_min`, `industry_naic_prefix`, `naics3` verified 2026-07-03; `industry_naic`, `policyholders_max` upstream-declared |
| `serff_filing` | $0.10 (gated) | Full SERFF filing detail by integer `filing_id`. 90-day dedupe vendor-documented in 2 written sources (unreceipted) |
...
- **Gated-tool switch:** set `XDATE_DISABLE_PAID=1` (also accepts `true` / `yes` / `on` / `enabled`, case-insensitive) in env to short-circuit the six gated tools — `company_details` ($0.25), `talkpoints` ($0.10), `serff_filing` ($0.10), `serff_search` (conflicting vendor pricing 2026-07-03: $0.05 per pricing page vs free per metadata — treated as paid), `group_companies` and `run_saved_search` (unverified stored-content executors) — with `isError` responses. Any other non-empty value leaves gated tools enabled and logs a startup warning to stderr. The seven always-free tools (`search`, `match`, `filter`, `benefits_search`, `flagged_companies`, `groups`, `saved_searches`) stay enabled; note the account-read tools among them still expose agency flag/pipeline lists to any connected client
...
- **Pricing sources now conflict.** The upstream tools/list metadata declares `serff_search` free, but the vendor's official pricing page (checked 2026-07-03, same day) lists it at $0.05/call — the vendor contradicts itself in writing. The wrapper treats serff_search as paid at $0.05 and keeps it behind the `XDATE_DISABLE_PAID` gate pending a billing receipt. The 90-day same-record dedupe on `company_details`/`talkpoints`/`serff_filing` is documented in BOTH written sources (metadata + pricing page) — stronger than a single declaration, still not billing-receipt-verified. The pricing page also says company_details includes "current flag/appointment status": the MCP route this wrapper uses does return that (`user_status`); the REST counterpart does not (see REST counterpart probes below).

## Pricing tested against our own account (2026-07-03)

XDate's published prices are not reliable — its own pricing page and tools/list metadata disagreed on `serff_search` the same day. The account exposes a live billing surface at `GET /api2/Account` (`apiBalance`, `apiFreeMonthly`), so pricing is now verified by balance-delta instead of vendor claims:

- **Nine paid-class calls on this account moved neither counter.** Fresh, never-pulled `company_details` and `talkpoints` on a random TX company, plus fresh `serff_search` and `serff_filing`, each showed a $0.0000 delta (`apiBalance` held $32.0000, `apiFreeMonthly` held $5.0000; `mcpAccess`=0, `mcpMonthlyBudget`=null). No delayed in-session posting was observed after the run.
- **So on this account the observed cost of every "paid" tool is currently $0.00**, contradicting the published $0.25 / $0.10 / $0.05. The published prices are retained in tool descriptions as a *caution ceiling* (worst case if the vendor flips metering, or if billing posts on a batch not visible in-session), and the gated tools stay gated: the mechanism is unconfirmed and one clean pass is not a fixture-backed de-gate.
- The mechanism is not established: `apiBalance` never moved during the test, so it could not be positively confirmed as the charge ledger. Re-read it in a later session to catch any delayed/batched posting. A companion helper lives in the operator's toolchain (not shipped in this repo) that reads only the four billing fields, never credentials.
...
- Whether an MCP pull and a REST pull of the same UID count as one record for the 90-day dedupe is unprobed; assume they may bill separately.
