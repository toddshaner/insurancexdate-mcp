...
export const CompanyDetailsSchema: Shape = {
  ...
  scope: z.array(z.enum(["details", "carriers", "contacts", "altloc", "tabs", "comments"])).optional()
    .describe("Data blocks to include. Default ['details','carriers']. VERIFIED in a paid response 2026-07-03 — note the scope values do NOT match the response keys: 'carriers' -> carrier_history; 'altloc' -> other_locations (array of {fein,name,address,city,state,zip}); 'tabs' -> THREE top-level blocks osha ({violationCount,violationCost,inspections}), benefits_health (Form 5500 Sch A), benefits_retirement (Form 5500) — there is no 'tabs' key in the response, and no DOT block was observed (test company had no DOT exposure; DOT content remains unverified and may require the vendor's enhanced-search add-on). 'contacts' -> contacts array with {name,firstname,lastname,email,position,phone,profileUrl,year} — restored via scope after disappearing from the default response in June 2026. 'comments' -> comments array (notes/flag history)."),
};
...
export const TOOL_DESCRIPTIONS = {
  ...
  company_details: "Full company details for a UID. Default response (observed 2026-06-12): summary (markdown top-line), user_status (CRM-style flag, often null), details (~77 fields incl. premium, payroll, mod/renmod, carrier, agent, signalScore, hazardGroup), carrier_history[] (full per-policy-term rows across years and states — can run to hundreds of rows for multi-state operators), _meta (per-field documentation). Optional `scope` blocks VERIFIED in a paid response 2026-07-03: 'contacts' returns a contacts[] array (name/email/position/phone/LinkedIn profileUrl) — contact data is BACK via scope after vanishing from the default response in June 2026; 'altloc' returns under the key `other_locations`; 'tabs' returns osha + benefits_health + benefits_retirement as top-level blocks (no 'tabs' key; no DOT block observed on a non-DOT risk — DOT content unverified and may require the vendor's enhanced-search add-on); 'comments' returns a comments[] array. Cost: $0.25/call; upstream declares repeat calls for the same company within 90 days free (dedupe — declared 2026-07-03, unverified against billing). Saving or caching forbidden by XDate terms.",
