...
Free read tools (`search`, `native_search`, `match`, `filter`, `benefits_search`, `account_status`, `flagged_companies`, `groups`, `saved_searches`) can be exercised only with authorized account data. `native_search` requires an explicit `datamode`; do not re-add the six excluded filters without a current paired probe proving the filter changes the expected cohort. `account_status` must retain its two-field output allowlist.

Gated tools charge your XDate account (ledger-confirmed 2026-07-03): `company_details` ($0.25), `talkpoints` ($0.10), and `serff_filing` ($0.10) charge per call with same-day same-record dedupe only — the advertised 90-day window failed a live test, so assume repeats across days bill again; `serff_search` is $0.05/call (despite vendor metadata claiming free) with NO repeat-call dedupe; `group_companies` and `run_saved_search` execute stored account content that could not be behavior-verified. Charges draw from a $5/month included allowance before the prepaid balance, so small test runs may not visibly dent the balance — check the account's XChange ledger, not the balance, when verifying costs. Gated tools default disabled; set `XDATE_DISABLE_PAID=0` only for an explicitly authorized paid test.

Never call `add_note` or `set_flag` against a live account as part of ordinary CI or PR verification. Their tests must use a stub client and prove that `XDATE_ENABLE_WRITES` fails closed, `confirm` is required and stripped, malformed input makes zero upstream calls, and no automatic retry occurs. Any live mutation needs a disposable test record and separate approval immediately before the call.
